...
$pdf->SetTitle('REPORTE DE INSUMOS UTILIZADOS');
...
$pdf->AddPage('L', 'LEGAL');
//celda para titulo
$pdf->Cell(0, 0, "REPORTE DE INSUMOS UTILIZADOS EN SERVICIOS", 0, 1, 'C');
//SALTO DE LINEA
$pdf->Ln();
...
$ordentrabajos = consultas::get_datos("select * from v_insumos_utilizados "
                . "where cod_insumo_uti=" . $_REQUEST['vinsu_uti'] . " order by cod_insumo_uti");
...
//            $pdf->Cell(40, 5, 'FECHA FIN',0 , 0, 'C', 1);
  ...
  $pdf->Ln();
        $pdf->SetFillColor(255, 255, 255);
        $pdf->SetFont('', '', 10);
            $pdf->Cell(30, 5, $ordentrabajo['cod_insumo_uti'], 0, 0, 'C', 1);
...
//           
        ...
        $detalles = consultas::get_datos("select * from v_deta_insumos_uti "
                        . "where cod_insumo_uti=" . $ordentrabajo['cod_insumo_uti'] . " order by cod_insumo_uti");
        if (!empty($detalles)) {
...
//            $pdf->Cell(30, 5, 'SERVICIO 2', 1, 0, 'C', 1);
       ...
       $pdf->Cell(20, 5, $detalle['cod_art'], 1, 0, 'C', 1);
                ...
                $pdf->Cell(30, 5, $detalle['cantidad_insu'], 1, 0, 'C', 1);
