...
$pdf->AddPage('L', 'LEGAL');
//celda para titulo
$pdf->Cell(0, 0, "REPORTE DE DESCUENTO EN SERVICIOS", 0, 1, 'C');
//SALTO DE LINEA
$pdf->Ln();
...
$ordentrabajos = consultas::get_datos("select * from v_descuento_servi "
                . "where cod_desc_servi=" . $_REQUEST['codigo'] . " order by cod_desc_servi");
...
//            $pdf->Cell(60, 5, 'CLIENTE',0 , 0, 'C', 1);
  ...
  $pdf->Ln();
        $pdf->SetFillColor(255, 255, 255);
        $pdf->SetFont('', '', 10);
            $pdf->Cell(30, 5, $ordentrabajo['cod_desc_servi'], 0, 0, 'C', 1);
...
//            $pdf->Cell(60, 5, $ordentrabajo['cliente'], 0, 0, 'C', 1);
            $pdf->Cell(40, 5, $ordentrabajo['fecha_ini'], 0, 0, 'C', 1);
            $pdf->Cell(40, 5, $ordentrabajo['fecha_fin'], 0, 0, 'C', 1);
             $pdf->Cell(30, 5, $ordentrabajo['estado'], 0, 0, 'C', 1);
//           
        $pdf->Ln();
        $pdf->Ln();

        $detalles = consultas::get_datos("select * from v_deta_descuento_servi "
                        . "where cod_desc_servi=" . $ordentrabajo['cod_desc_servi'] . " order by cod_desc_servi");
        if (!empty($detalles)) {
       ...
       $pdf->Cell(20, 5, $detalle['cod_desc_servi'], 1, 0, 'C', 1);
                ...
                $pdf->Cell(40, 5, $detalle['valor_porc_desc']." %", 1, 0, 'C', 1);
                $pdf->Cell(30, 5, number_format(($detalle['precio_tipo_servi']),0,',','.'), 1, 0, 'C', 1);
                $pdf->Cell(30, 5, number_format(($detalle['descuento_servi']),0,',','.'), 1, 0, 'C', 1);
