...
$sqlventas = "select *,(select numero_letras(total_devolucion)) "
        . "as total_letra "
      . " from v_nota_cred_imprimir_fact where cod_not_credi_vent = " . 
        $_REQUEST ['vcod'] . "";
...
$pdf->Cell(85, 1, 'Servicio y Mantenimiento de Motos', 0, 0, 'C');

$pdf->Cell(100, 1, 'NRO.: ' . $rsventas[0]['nro_nota'], 0, 1, 'C');
$pdf->SetFont('Times', 'B', 12);
...
//$pdf->Cell(80, 1, 'VIGENCIA FIN.: ' . $rsventas[0]['fecha_fin'], 0, 1, 'C');
$pdf->Cell(70, 1, 'VIG. INICIO.: ' . $rsventas[0]['fecha_inicio'].' - FIN.: ' . $rsventas[0]['fecha_fin'], 0, 0, 'C');
...
$pdf->Cell(/*1*/140, /*2*/1, /*3*/$rsventas[0]['fecha_credi']."  -  N° FACTURA : ".$rsventas[0]['nro_factura']."  - CONDICION: ".$rsventas[0]['ven_condicion']. "  -  MONTO: ".number_format(($rsventas[0]['ven_total']),0,',','.'), /*4*/0, /*5*/1, /*6*/'L', /*7*/null, /*8*/null, /*9*/1, /*10*/null, /*11*/null, /*12*/null);
$pdf->Ln(3);
//nombre cliente
$pdf->SetFont('Times', 'B', 10);
...
$pdf->SetFont('Times', '', 10);
$pdf->Cell(/*1*/37, /*2*/1, /*3*/$rsventas[0]['credi_descuento']." % ", /*4*/0, /*5*/0, /*6*/'L', /*7*/null, /*8*/null, /*9*/1, /*10*/null, /*11*/null, /*12*/null);
$pdf->Ln(6);
...
//$pdf->Cell(25, 1, '', 0, 0, 'L');
//$pdf->Cell(25, 1, '', 0, 0, 'L');
$pdf->Cell(30, 1, '   MOTIVO: ', 0, 0, 'L');
$pdf->SetFont('Times', '', 10);
$pdf->Cell(/*1*/40, /*2*/1, /*3*/$rsventas[0]['descri_motivo'], /*4*/0, /*5*/0, /*6*/'L', /*7*/null, /*8*/null, /*9*/1, /*10*/null, /*11*/null, /*12*/null);
//cuadro de detalles
...
$pdf->SetFont('Times', 'B', 10);
$pdf->Cell(15, 1, '#', 0, 0, 'C');
$pdf->Cell(35, 1, 'Item', 0, 0, 'L');
$pdf->Cell(35, 1, 'Servicio', 0, 0, 'L');
$pdf->Cell(15, 1, 'Cant. Devol.', 0, 0, 'C');
$pdf->Cell(25, 1, 'Precio Item', 0, 0, 'R');
$pdf->Cell(25, 1, 'Monto Serv', 0, 0, 'R');
$pdf->Cell(25, 1, 'Subtotal', 0, 0, 'R');
$pdf->Ln(5);

$consultas = "select * from v_notacredito_fac_imprimir_detalle_2 where cod_not_credi_vent=".$_REQUEST ['vcod'];
$detventas = consultas::get_datos($consultas);
if (!empty($detventas)) {
foreach ($detventas as $report) {
    $pdf->SetFont('Times', '', 10);
    $pdf->Cell(15, 1, $report['cod_not_credi_vent'], 0, 0, 'C');
    $pdf->Cell(35, 1, /*3*/$report['art_descri'], 0, 0,'L',null,null,1,null,null,null);
    $pdf->Cell(35, 1, /*3*/$report['descri_servicio'], 0, 0,'L',null,null,1,null,null,null);
    $pdf->Cell(15, 1,$report['det_canti_devolucion'] , 0, 0, 'C');
    $pdf->Cell(25, 1, number_format(($report['det_nota_precio']),0,',','.') , 0, 0, 'R'); 
    $pdf->Cell(25, 1, number_format(($report['monto_servicio']),0,',','.') , 0, 0, 'R'); 
     $pdf->Cell(25, 1, number_format(($report['sub_cal_devolucion']),0,',','.'), 0, 1, 'R');
}} else {
...
$pdf->SetFont('Times', '', 10);
$pdf->Text(165, 243, 'Gs. '.
        number_format(($rsventas[0]['total_devolucion']),0,',','.'));
$pdf->SetFont('Times', 'B', 10);
...
$pdf->SetFont('Times', '', 10);
$pdf->Text(100, 243,
        number_format(($rsventas[0]['total_iva_10']),0,',','.'));
$pdf->SetFont('Times', 'B', 10);
